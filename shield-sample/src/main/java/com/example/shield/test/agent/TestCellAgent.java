...
import com.example.shield.test.cell.MixCell;

import rx.Subscription;
import rx.functions.Action1;
import rx.functions.Func1;

public class TestCellAgent extends LightAgent {
    ...
    public TestCellAgent(Fragment fragment, DriverInterface bridge, PageContainerInterface pageContainer) {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        loadingSubscription = getWhiteBoard().getObservable(TestLoadingAgent.KEY_LOADING).filter(new Func1() {
            @Override
        ...
        emptySubscription = getWhiteBoard().getObservable(TestLoadingAgent.KEY_EMPTY).filter(new Func1<Object, Boolean>() {
            @Override
        ...
        failedSubscription = getWhiteBoard().getObservable(TestLoadingAgent.KEY_FAILED).filter(new Func1<Object, Boolean>() {
            @Override
            public Boolean call(Object o) {
                return o instanceof Boolean && ((Boolean) o);
            }
        ...
        moreSubscription = getWhiteBoard().getObservable(TestLoadingAgent.KEY_MORE).filter(new Func1<Object, Boolean>() {
            @Override
            public Boolean call(Object o) {
                return o instanceof Boolean && ((Boolean) o);
            }
